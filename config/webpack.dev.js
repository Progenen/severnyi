...
if (!fs.existsSync(builFolder)) {
  fs.mkdirSync(builFolder);
}

// Create the response folder in the build folder
const responseFolderPath = path.join(builFolder, responseFolder);
fs.mkdirSync(responseFolderPath);

// Copy all files from the response folder in the src folder to the response folder in the build folder
const srcResponseFolderPath = path.join(srcFolder, responseFolder);
fs.readdirSync(srcResponseFolderPath).forEach((file) => {
  const srcFilePath = path.join(srcResponseFolderPath, file);
  const destFilePath = path.join(responseFolderPath, file);
